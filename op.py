...
class ReadRate(object):
    ...
    def make_bins_read_cross(self,var,atm):
        ...
        sp_label = np.genfromtxt(self.cfg.cross_folder+'thresholds.txt',dtype=str, usecols=0) # taking the first column as labels
        lmd_data = np.genfromtxt(self.cfg.cross_folder+'thresholds.txt', skip_header = 1)[:,1] # discarding the fist column
        ...
        for n, sp in enumerate(absp_sp):
            ...
            if self.cfg.use_ion:
                try:
                    cross_raw[sp] = np.genfromtxt(self.cfg.cross_folder+sp+'/'+sp+'_cross.csv',dtype=float,delimiter=',',skip_header=1, names = ['lambda','cross','disso','ion'])
                except:
                    print ('\nMissing the cross section from ' + sp); raise
                if sp in ion_sp:
                    try:
                        ion_ratio_raw[sp] = np.genfromtxt(self.cfg.cross_folder+sp+'/'+sp+'_ion_branch.csv',dtype=float,delimiter=',',skip_header=1, names = True)
                    except:
                        print ('\nMissing the ion branching ratio from ' + sp); raise
            else:
                try:
                    cross_raw[sp] = np.genfromtxt(self.cfg.cross_folder+sp+'/'+sp+'_cross.csv',dtype=float,delimiter=',',skip_header=1, names = ['lambda','cross','disso'])
                except:
                    print ('\nMissing the cross section from ' + sp); raise
            ...
            if sp in photo_sp: # excluding ion_sp
                try:
                    ratio_raw[sp] = np.genfromtxt(self.cfg.cross_folder+sp+'/'+sp+'_branch.csv',dtype=float,delimiter=',',skip_header=1, names = True)
                except:
            ...
            if sp in self.cfg.T_cross_sp:
                ...
                for tt in T_list:
                    if self.cfg.use_ion: # usually the T-dependent cross sections are only measured in the photodissociation-relavent wavelengths so cross_tot = cross_diss
                        cross_T_raw[(sp, tt)] = np.genfromtxt(self.cfg.cross_folder+sp+'/'+sp+'_cross_'+str(tt)+'K.csv',dtype=float,delimiter=',',skip_header=1, names = ['lambda','cross','disso','ion'])
                    else:
                        cross_T_raw[(sp, tt)] = np.genfromtxt(self.cfg.cross_folder+sp+'/'+sp+'_cross_'+str(tt)+'K.csv',dtype=float,delimiter=',',skip_header=1, names = ['lambda','cross','disso'])
        ...
        for sp in self.cfg.scat_sp:
            scat_raw[sp] = np.genfromtxt(self.cfg.cross_folder + 'rayleigh/' + sp+'_scat.txt',dtype=float,\
